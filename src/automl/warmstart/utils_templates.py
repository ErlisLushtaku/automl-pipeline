...
def hyperparameter_context(config_space: ConfigurationSpace, add_ranges=False):
    str_result = ''
    for hp in config_space.get_hyperparameters():
        ...
        if isinstance(hp, CS.CategoricalHyperparameter):
            ...
            if choices[0] == False and choices == 'True':
                ...
                str_result += ' (string), ' if not add_ranges else ' (string %s), ' % str(choices).replace('[',
                                                                                                           '{').replace(
                    ']', '}')
        else:
...
def add_context_information(text, config_space: ConfigurationSpace, num_recommendation=10, text_dict=None, provide_ranges=False):
...
class TemplateReader(ABC):
    ...
    def add_context(self, config_space: ConfigurationSpace, num_recommendation=10, task_dict=None):
...
class FullTemplate(TemplateReader):
    ...
    def obtain_dict_text(self, task_dict):
        ...
        new_dict['[IMAGE_SIZE]'] = str(task_dict['image_size'])
        # new_dict['[NUM_FEATURES]'] = str(task_dict['num_feat'])
        # new_dict['[NUM_NUM_FEATURES]'] = str(task_dict['num_feat'])
        ...
        new_dict['[DESCRIPTION]'] = str(task_dict['description'])
        return new_dict
